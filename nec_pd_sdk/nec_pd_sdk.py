...
# TODO == ATHLON 5 ==
# Check: command_auto_id_complete_notify
# Check: command_auto_tile_matrix_complete
# Check: command_tile_matrix_profile_contents_write
# Check: command_tile_matrix_profile_contents_read
# Check: command_tile_matrix_profile_write
# Check: command_pbp_pip_profile_contents_write
# Check: command_pbp_pip_profile_contents_read
# Check: command_pbp_pip_profile_write
...
# ------------------  MEDIA PLAYER COMMANDS -----------------
    ...
    @retry
    def command_cmd_get_mp_slideshow_interval(self):
        """
        ...
        logging.debug('')
        send_data = []
        send_data.extend(protocol.ascii_encode_value_4_bytes(0xCA1D))
        send_data.extend(protocol.ascii_encode_value_2_bytes(0x02))
        protocol.write_command(self.f, send_data, self.destination_address, 0x41)
        reply_data, reply_message_type, reply_destination_address = protocol.read_command_reply(self.f, True)
        if len(reply_data) == 12:
            if reply_message_type != 0x42:
                ...
                raise unexpectedReply
            # reply
            if reply_data[0:4] != protocol.ascii_encode_value_4_bytes(0xCB1D):
                logging.error('unexpected reply received')
                raise unexpectedReply
            if reply_data[4:6] != protocol.ascii_encode_value_2_bytes(0x00):
                logging.error('reply status is not 0')
            ...
            if reply_data[6:8] != protocol.ascii_encode_value_2_bytes(0x02):
                logging.error('unexpected reply received')
                raise unexpectedReply
            return protocol.ascii_decode_value(reply_data[8:12])
        else:
            logging.error('unexpected reply length: %i (expected 12)', len(reply_data))
            raise unexpectedReply

    @retry
    def command_cmd_set_mp_slideshow_interval(self, value):
        """

        :return:
        """
        logging.debug('')
        assert 0x0005 <= value <= 0x012C
        send_data = []
        send_data.extend(protocol.ascii_encode_value_4_bytes(0xCA1E))
        send_data.extend(protocol.ascii_encode_value_2_bytes(0x02))
        send_data.extend(protocol.ascii_encode_value_4_bytes(value))
        protocol.write_command(self.f, send_data, self.destination_address, 0x41)
        reply_data, reply_message_type, reply_destination_address = protocol.read_command_reply(self.f, True)
        if len(reply_data) == 8:
            if reply_message_type != 0x42:
                ...
                raise unexpectedReply
            # reply
            if reply_data[0:4] != protocol.ascii_encode_value_4_bytes(0xCB1E):
                logging.error('unexpected reply received')
                raise unexpectedReply
            if reply_data[4:6] != protocol.ascii_encode_value_2_bytes(0x00):
                logging.error('reply status is not 0')
    ...
    @retry
    def command_cmd_get_mp_slideshow_repeat_on_off(self):
        """
        ...
        if len(reply_data) == 10:
            ...
            if reply_data[4:6] != protocol.ascii_encode_value_2_bytes(0x00):
                logging.error('reply status is not 0')
                raise commandStatusReturnedError
            if reply_data[6:8] != protocol.ascii_encode_value_2_bytes(0x03):
                logging.error('unexpected reply received')
                raise unexpectedReply
            return protocol.ascii_decode_value(reply_data[8:10])
        else:
    ...
    @retry
    def command_cmd_set_mp_slideshow_repeat_on_off(self, value):
        """

        :return:
        """
        logging.debug('')
        assert 0x00 <= value <= 0x01
        send_data = []
        send_data.extend(protocol.ascii_encode_value_4_bytes(0xCA1E))
        send_data.extend(protocol.ascii_encode_value_2_bytes(0x03))
        send_data.extend(protocol.ascii_encode_value_2_bytes(value))
    ...
    @retry
    def command_cmd_get_mp_autoplay(self):
        """
        ...
        send_data = []
        send_data.extend(protocol.ascii_encode_value_4_bytes(0xCA1D))
        send_data.extend(protocol.ascii_encode_value_2_bytes(0x0A))
        protocol.write_command(self.f, send_data, self.destination_address, 0x41)
        reply_data, reply_message_type, reply_destination_address = protocol.read_command_reply(self.f, True)
        if len(reply_data) == 10:
            if reply_message_type != 0x42:
            ...
            if reply_data[4:6] != protocol.ascii_encode_value_2_bytes(0x00):
                logging.error('reply status is not 0')
                raise commandStatusReturnedError
            if reply_data[6:8] != protocol.ascii_encode_value_2_bytes(0x0A):
                logging.error('unexpected reply received')
                raise unexpectedReply
            return protocol.ascii_decode_value(reply_data[8:10])
        else:
    ...
    @retry
    def command_cmd_set_mp_autoplay(self, value):
        """
    ...
    @retry
    def command_cmd_get_mp_autoplay_folder(self, block):
        """

        :return:
        """
        logging.debug('')
        assert 0x01 <= block <= 0xff
        send_data = []
        # recv_data = []
        recv_data = bytearray()
        send_data.extend(protocol.ascii_encode_value_4_bytes(0xCA1D))
        send_data.extend(protocol.ascii_encode_value_2_bytes(0x0B))
        send_data.extend(protocol.ascii_encode_value_2_bytes(block))
        protocol.write_command(self.f, send_data, self.destination_address, 0x41)
        reply_data, reply_message_type, reply_destination_address = protocol.read_command_reply(self.f, True)
        if len(reply_data) >= 12:
            if reply_message_type != 0x42:
            ...
            if reply_data[4:6] != protocol.ascii_encode_value_2_bytes(0x00):
                logging.error('reply status is not 0')
                raise commandStatusReturnedError
            if reply_data[6:8] != protocol.ascii_encode_value_2_bytes(0x0B):
                logging.error('unexpected reply received')
                raise unexpectedReply
            all_block = protocol.ascii_decode_value(reply_data[8:10])
            if reply_data[10:12] != protocol.ascii_encode_value_2_bytes(block):
                logging.error('unexpected reply received')
            pos = 12
            while pos < len(reply_data):
                x = protocol.ascii_decode_value(reply_data[pos:pos + 2])
                ...
                pos += 2
                recv_data.append(x)
            return all_block, recv_data #  .decode('utf-8')
        else:
            logging.error('unexpected reply length: %i (expected at least 12)', len(reply_data))
            raise unexpectedReply

    @retry
    def helper_command_cmd_get_mp_autoplay_folder(self):
        reply = self.command_cmd_get_mp_autoplay_folder(1)
        print (reply)
        for block in range(2, reply[0]+1):
            print("block:", block)
            reply2 = self.command_cmd_get_mp_autoplay_folder(block)
            reply[1].extend(reply2[1])
    ...
    @retry
    def command_cmd_set_mp_autoplay_folder(self, block, all_block, string_byte_array):
        """
        ...
        send_data.extend(protocol.ascii_encode_value_4_bytes(0xCA1E))
        send_data.extend(protocol.ascii_encode_value_2_bytes(0x0B))
        send_data.extend(protocol.ascii_encode_value_2_bytes(all_block))
        send_data.extend(protocol.ascii_encode_value_2_bytes(block))
        for byte in string_byte_array:
            send_data.extend(protocol.ascii_encode_value_2_bytes(byte))
        ...
        if len(reply_data) == 10:
            ...
            if reply_data[6:8] != protocol.ascii_encode_value_2_bytes(0x0B):
                logging.error('unexpected reply received')
                raise unexpectedReply
            if reply_data[8:10] != protocol.ascii_encode_value_2_bytes(block):
                logging.error('unexpected reply received')
            return
        else:
            logging.error('unexpected reply length: %i (expected 10)', len(reply_data))
    ...
    @retry
    def helper_command_cmd_set_mp_autoplay_folder(self, in_string):
        string_byte_array = bytearray(in_string.encode('utf-8'))
        if string_byte_array[-1] is not 0x00:
            string_byte_array.extend([0x00])
        length = len(string_byte_array)
        all_block = 1 + length / 240
        print ("length", length)
        print("all_block", all_block)
        ...
        while remaining_length > 0:
            ...
            this_block_length = min(240, remaining_length)
            print("this_block_length", this_block_length)
            print("position", position)
            self.command_cmd_set_mp_autoplay_folder(block_offset, all_block, string_byte_array[position:position+this_block_length])
            remaining_length -= this_block_length
            position += this_block_length
            block_offset += 1
